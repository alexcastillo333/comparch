...
void put(char *id, node_t *nptr) {
    ...
    if (var_table->entries[index] == NULL) {
        ...
        entry_t * bucket = var_table->entries[index]->next;
        entry_t * trailer = var_table->entries[index];
        if (strcmp(trailer->id, id) == 0) {
            if (nptr->type == STRING_TYPE) {
                delete_entry(trailer);
                var_table->entries[index] = init_entry(id, nptr);
                var_table->entries[index]->next = bucket;
            } else {
                trailer->type = nptr->type;
                trailer->val.ival = nptr->val.ival;
            }
            return;   
        }
        while (bucket != NULL) {
            if (strcmp(bucket->id, id) == 0) {
                if (nptr->type == STRING_TYPE) {
                    entry_t * temp = bucket->next;
                    delete_entry(bucket);
                    trailer->next = init_entry(id, nptr);
                    trailer->next->next = temp;
                } else {
                    bucket->type = INT_TYPE;
                    bucket->val.ival = nptr->val.ival;
                }
                return;   
            }
            bucket = bucket->next;
...
entry_t* get(char* id) {
    ...
    int index = hash_function(id);
    entry_t * bucket = var_table->entries[index];
    while (bucket != NULL) {
        if (strcmp(bucket->id, id) == 0) {
            return bucket;
        }
