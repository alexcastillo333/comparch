...
void do_decode_stage()
{
    ...
    //set destM
    if(decode_output -> icode == I_MRMOVQ || decode_output -> icode == I_POPQ) {
...
void do_execute_stage()
{
    ...
    if ((execute_output->icode) == (I_ALU)) {
        setcc = true;
        if (memory_output->status == STAT_ADR || memory_output->status == STAT_INS ||
                memory_output->status == STAT_HLT) {
            setcc = false;
        }
    }
    ...
    else if(execute_output -> icode == I_RET || execute_output -> icode == I_POPQ) {
        alua = 8;
    }
    else {
        alua = 0;
    ...
    if(execute_output->icode == I_RRMOVQ) {
        if(e_bcond) {
            alua = execute_output->vala;
        } else {
            alua = execute_output->valb;
        }
    }
...
void do_memory_stage()
{
    ...
    if((memory_output->icode) == (I_MRMOVQ) || (memory_output->icode) == (I_POPQ) || (memory_output->icode) == (I_RET)) {
        mem_read = true;
    }

    //set mem_addr
    if((memory_output->icode) == (I_RMMOVQ) || (memory_output->icode) == (I_PUSHQ) || (memory_output->icode) == (I_CALL) || (memory_output->icode) == (I_MRMOVQ)) {
        mem_addr = memory_output -> vale;
    }
    else if ((memory_output->icode) == (I_POPQ) || (memory_output->icode) == (I_RET)) {
    ...
    //set mem_write
    if((memory_output->icode) == (I_RMMOVQ) || (memory_output->icode) ==  (I_PUSHQ) || (memory_output->icode) == (I_CALL)) {
        mem_write = true;
    }
...
void do_stall_check()
{
    ...
    if (memory_input->status == STAT_ADR || memory_input->status == STAT_INS) {
        memory_state->op = pipe_cntl("MEM", false, true);
    }

    if (writeback_input->status == STAT_ADR || writeback_input->status == STAT_INS) {
        memory_state->op = pipe_cntl("WB", true, false);
